...
Run from any directory: python scripts/smoke_wheel.py dist/calltrail-*.whl
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='calltrail-wheel-') as directory:
        root = Path(directory)
        environment = root/'environment'
        venv.EnvBuilder(with_pip=True).create(environment)
        binary = environment/('Scripts' if os.name == 'nt' else 'bin')
        python = binary/('python.exe' if os.name == 'nt' else 'python')
        cli = binary/('calltrail.exe' if os.name == 'nt' else 'calltrail')
        ...
        location = run([python,'-I','-c','import calltrail; print(calltrail.__file__)']).stdout.strip()
        assert Path(location).is_relative_to(environment), location
        project = root/'fixture'
        project.mkdir()
        (project/'.calltrail.toml').write_text('[commands]\ncheck = '+json.dumps([str(python),'-c','print("wheel works")'])+'\n')
