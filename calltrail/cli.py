...
from calltrail import __version__
from calltrail.changes import collect_changes
from calltrail.commands import discover_commands
from calltrail.config import ConfigError, CallTrailConfig
from calltrail.detectors.project import detect_project_name, find_project_root
...
from calltrail.impact import affected_commands, impact_report
...
def parser() -> argparse.ArgumentParser:
    cli = argparse.ArgumentParser(
        prog="calltrail", description="Inspect your project, run tasks, and monitor your development environment.",
        epilog="Commands run only when requested. Use --list-commands for tasks; calltrail trace --help for static Python call tracing.",
    )
    ...
    mode.add_argument("--init", action="store_true", help="create .calltrail.toml without overwriting an existing file")
...
def initialize(root: Path) -> Path:
    ...
    lines = ["# CallTrail runs commands only when you request them.", "[project]",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if argv and argv[0] == 'trace' and (
        ...
        from calltrail.trace.cli import main as trace_main
    ...
    try:
        ...
        if args.init:
            ...
            return 0
        config = CallTrailConfig.load(root)
        commands = discover_commands(root, config)
        if args.affected or args.run_affected:
            ...
            return 0
        if args.doctor:
            from calltrail.doctor import doctor_report
        ...
        if args.list_commands:
            if not commands:
                print("No commands detected. Add [commands] to .calltrail.toml, or run calltrail --init.")
            for name, command in commands.items():
        ...
        app = CallTrailApp(root, interval=args.interval, timeout=args.timeout)
        app.run()
        ...
        print(f"calltrail: {exc}", file=sys.stderr)
        return 2
