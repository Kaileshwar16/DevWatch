...
from calltrail.outcomes import ExecutionReason as Reason, ExecutionResult, ExecutionState as State
from calltrail.runner import project_environment
...
def go_discovery_hazards(root: Path, limit: int = 10000) -> tuple[list[str], bool]:
    """Bounded Go-like walk. Never apply CallTrail's generated-directory exclusions.
...
def _node_prerequisite(command: Command, cwd: Path, env: dict[str, str], result: CommandPreflight) -> None:
    from calltrail.metadata import read_json
    argv = command.argv
...
def preflight_command(command: Command, root: Path, *, thorough: bool = True) -> CommandPreflight:
    ...
    try:
        from calltrail.config import command_args
