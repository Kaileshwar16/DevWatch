...
from calltrail.tasks import TaskManager, TaskRun
...
from calltrail.widgets.docker_panel import DockerPanel
from calltrail.widgets.git_panel import GitPanel
from calltrail.widgets.ports_panel import PortsPanel
from calltrail.widgets.project_panel import ProjectPanel
from calltrail.widgets.test_panel import TestPanel


class CallTrailApp(App):
    ...
    def compose(self) -> ComposeResult:
        yield Static("calltrail", id="titlebar", markup=False)
    ...
    def action_commands(self) -> None:
        if not self._commands:
            self.notify("No commands detected. Add [commands] to .calltrail.toml.", severity="warning")
            return
    ...
    def _start_command(self, name: str | None) -> None:
        ...
        if name not in self._commands:
            self.notify("No test command detected. Add [commands] test to .calltrail.toml.", severity="warning")
            return
        try:
...
def main() -> int:
    ...
    from calltrail.cli import main as cli_main
    return cli_main()
